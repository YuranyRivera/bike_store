...
import '../CatAccesorios/CatAccesorios.css';
...
function Catalogo() {
        ...
        useEffect(() => {
          async function fetchProductos() {
            try {
              const response = await fetch('http://localhost:4000/api/Accesorios');
              if (!response.ok) {
              ...
              {productos.map(producto => (
                <div className="box-galery" key={producto.id_articulo}>
                  <div className="box-img1">
                    <img className="a" src={`src/assets/img/BIKE_STORE/${producto.categoria_descripcion}/${producto.imagen}/casco1.png`} alt={producto.nombre} />
                  </div>
